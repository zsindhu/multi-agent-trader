...
// ── Markdown renderer for AI reasoning ───────────────────────────────
const mdComponents = {
  ...
  return (
    <div className="border-l-2 border-purple-500/50 bg-purple-500/5 rounded-r-lg p-3 border border-l-0 border-purple-500/15">
      ...
      {open && entry.reasoning && (
        <div className="mt-2 p-3 bg-slate-900/60 rounded overflow-auto max-h-48 border border-purple-500/10">
          <ReactMarkdown remarkPlugins={[remarkGfm]} components={mdComponents}>{entry.reasoning}</ReactMarkdown>
        </div>
      )}
    </div>
  )
