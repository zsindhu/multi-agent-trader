...
class CoveredCallWorker(BaseAgent):
        ...
        for trade in trades:
            try:
                ...
                if not trade.get("option_symbol"):
                    ...
                    logger.error(f"[CC] Skipping trade — option_symbol is None/empty for {trade.get('symbol')}")
                    continue
                if not trade.get("limit_price") or trade["limit_price"] <= 0:
                    ...
                    logger.error(f"[CC] Skipping trade — limit_price={trade.get('limit_price')} for {trade.get('symbol')} {trade.get('option_symbol')}")
                    continue
                ...
                try:
                    rationale = self.build_cc_rationale(
                        symbol=trade["symbol"],
                        strike=trade["strike"],
                        expiration=trade["expiration"],
                        ...
                        avg_cost=trade.get("avg_cost", 0),
                        downside_protection=trade.get("downside_protection", 0),
                        qty=trade["qty"],
                    )
    ...
    async def roll_position(self, option_symbol: str, reason: str = ""):
        """Public: roll a specific position by option symbol."""
        pos = next(
            (p for p in self.portfolio.options if p.option_symbol == option_symbol),
            None,
        )
